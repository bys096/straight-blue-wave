...
const ProjectList = () => {
	...
	const addUserFriend = async (id) => {
		await axios
			...
			.then((res) => {
				const findMember = res.data.dtoList.some((item) => item.memberId === id);
				if (findMember) {
	...
	const fetchProjects = async () => {
		try {
			const response = await axios.get(
				`http://localhost:8002/api/project/list/${sessionStorage.getItem("tmid")}`
			);
	...
	const whoIsAdmin = async () => {
		await axios
			.get(
				`http://localhost:8002/api/team/readTeam/${sessionStorage.getItem("tmid")}`
			)
			.then((res) => {
				if (
					parseInt(res.data.memberId) === parseInt(sessionStorage.getItem("memid"))
				) {
					setShowButton(true);
				}
			})
